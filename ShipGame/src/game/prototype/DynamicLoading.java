...
public class DynamicLoading {
	...
	public void update(Point2D.Float cameraPos, ArrayList<float[]> coords, ArrayList<int[]> colorRGB, float size) {		
		for (int i = 0; i < coords.size(); i++) {
			//Load on screen objects
			if (coords.get(i)[0] > cameraPos.x && coords.get(i)[0] < cameraPos.x + Game.WIDTH 
				&& coords.get(i)[1] > cameraPos.y && coords.get(i)[1] < cameraPos.y + Game.HEIGHT) {
				...
				if (colorRGB.get(i)[0] ==   0 && colorRGB.get(i)[1] == 255 && colorRGB.get(i)[2] ==   0 && coords.get(i)[2] == 0) 
					handler.addObject(new ExplosiveMine(coords.get(i)[0], coords.get(i)[1], size, size, handler, ObjectId.ExplosiveMine));
			...
			if (coords.get(i)[2] != 0) {	
				for (int j = 0; j < handler.object.size(); j++) {
					...
					if (tempObject.getX() < cameraPos.x || tempObject.getX() > cameraPos.x + Game.WIDTH) 		
						handler.removeObject(tempObject);									
					if (tempObject.getY() < cameraPos.y || tempObject.getY() > cameraPos.y + Game.HEIGHT) 
						handler.removeObject(tempObject);
				}			
			}			
		}
